...
void Game::InitPhysics()
{
    ...
    //Creamos superficie elastica
    b2FixtureDef suelo;
    suelo.restitution = 1;

    b2PolygonShape sueloShape;
    sueloShape.SetAsBox(100, 5);
    suelo.shape = &sueloShape;

    b2Fixture sueloFixture = *groundBody->CreateFixture(&suelo);
    ...
    techoShape.SetAsBox(100, 5);
    techo.shape = &techoShape;

    b2Fixture techoFixture = *ceilingBody->CreateFixture(&techo);
  
    b2Body* leftWallBody = Box2DHelper::CreateRectangularStaticBody(phyWorld, 10, 100);
    leftWallBody->SetTransform(b2Vec2(0.0f, 50.0f), 0.0f);

    //Creamos pared elastica
    b2FixtureDef pared;
    pared.restitution = 1;

    b2PolygonShape paredShape;
    paredShape.SetAsBox(5, 100);
    pared.shape = &paredShape;

    b2Fixture paredFixture = *leftWallBody->CreateFixture(&pared);
    ...
    //Creamos pared elastica
    b2FixtureDef pared2;
    pared2.restitution = 1;

    b2PolygonShape pared2Shape;
    pared2Shape.SetAsBox(5, 100);
    pared2.shape = &pared2Shape;

    b2Fixture pared2Fixture = *rightWallBody->CreateFixture(&pared2);

    // Crea un cuerpo de círculo controlado por el teclado
    controlBody = Box2DHelper::CreateCircularDynamicBody(phyWorld, 5, 1.0f, 0.5, 0.5f);
